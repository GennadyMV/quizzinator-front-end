var TEMPLATE = (function(){
	...
	var templates = {
		...
		open_question: '<div class="form-group quiz-item" data-type="open_question"><label class="open-question-question">{{question}}</label><textarea class="form-control open-question-value"></textarea></div>',
		multiple_choice_question: '<div class="form-group quiz-item" data-type="multiple_choice_question"><label class="multiple-choice-question-question" style="margin-bottom: 0px">{{question}}</label>{{#each options}}<div class="radio"><label><input type="radio" name="choice" value="{{title}}">{{title}}</label></div>{{/each}}',
		text_container: '<div class="form-group">{{{content}}}</div>',
